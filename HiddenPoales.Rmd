...
trait11<-trait11[!is.na(trait11[,2]),]
tipstoremove <- setdiff(tree$tip.label,trait11[,1])
mytree <- drop.tip(tree, tipstoremove)

pp.polymorphic11<-corHMM(mytree,trait11,rate.cat=2,node.states="marginal")
pp.polymorphic11
pp.polymorphic11.1<-corHMM(mytree,trait11,rate.cat=1,node.states="marginal")
pp.polymorphic11.1
pp.polymorphic11.2<-corHMM(mytree,trait11,rate.cat=2, model = "SYM", node.states="marginal")
pp.polymorphic11.2
pp.polymorphic11.3<-corHMM(mytree,trait11,rate.cat=1,model = "SYM", node.states="marginal")
pp.polymorphic11.3

pdffn = paste0("corHMMmodel_C3C4CAM.pdf")
pdf(file=pdffn, width=8, height=8)
plotMKmodel(pp.polymorphic11, rate.cat = 2, color = "col.blind" )
dev.off()

cols <- c("white", "black", "grey", "red", "yellow", "orange")
pdffn = paste0("polymorphicSYMandR1R2rates_C3C4CAM.pdf")
pdf(file=pdffn, width=6, height=200)
plotRECON(mytree,pp.polymorphic11$states,piecolors=cols, cex=0.2, pie.cex=0.3, show.tip.label=TRUE, title="Polymorphic ARD and R1/R2 rates")
dev.off()

trait <- trait11[,2]
names(trait) <- trait11[,1]
...
nodelabels(node=1:mytree$Nnode+Ntip(mytree),
    pie=pp.polymorphic11$states,piecol=cols,cex=0.3)
...
tiplabels(pie=to.matrix(trait[mytree$tip.label],
    levels(as.factor(trait))),piecol=cols,cex=0.1)
nodelabels(node=1:mytree$Nnode+Ntip(mytree),
    pie=pp.polymorphic11$states,piecol=cols,cex=0.1)
add.simmap.legend(colors=cols,prompt=FALSE,x=0.9*par()$usr[1],
    y=0.8*par()$usr[3],fsize=2)
dev.off()
